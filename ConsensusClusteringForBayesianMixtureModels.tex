...
\newcommand{\commentSC}[1]{{\color{black}#1}}
...
%finds the generating structure across our simulation studies and captures multiple modes in the likelihood surface.  This approach also 
...
%traditional Bayesian inference when a parallel environment is available. 
...
%, showing consensus clustering can be applied to any MCMC-based clustering method
...
%Consensus clustering enables use of existing implementations of MCMC-based clustering methods on high-dimensional datasets, performing meaningful, reproducible inference where traditional approaches fail and faster inference where they do not. 
This enables researchers to 
%use state-of-the-art Bayesian clustering methods 
...
%on modern 'omics datasets, methods that can jointly model multiple datasets and can infer the number of clusters present.
\end{abstract}
...
%\author{Stephen Coleman\,$^{\text{\sfb 1}\textrm{\ddagger}}$, Paul DW Kirk\,$^{\text{\sfb 1, \sfb 2}\dagger}$ and Chris Wallace\,$^{\text{\sfb 1,\sfb 2}\dagger}$}
...
%\abstract{\textbf{Motivation:} 
%Cluster analysis is an integral part of precision medicine and systems biology,
%used to define groups of patients or biomolecules.
%%and plays a role in more general analyses.
...
%\textbf{Results:} 
%%We investigate the performance of this approach in simulations, comparing it to Bayesian inference of the same models and \texttt{Mclust}, a popular implementation of MLE inference of mixture models in R. 
%Consensus clustering of Bayesian mixture models successfully finds generating structure in our simulation study and captures multiple modes in the likelihood surface. This approach also offers significant reductions in runtime compared to
%% use of a single long chain for 
%traditional Bayesian inference when a parallel environment is available. We propose a heuristic to decide upon ensemble size and then apply consensus clustering to Multiple Dataset Integration, an extension of Bayesian mixture models for integrative analyses, on three 'omics datasets for budding yeast. We find clusters of genes that are co-expressed and have common regulatory proteins which we validate using external knowledge, showing consensus clustering can be applied to any MCMC-based clustering method.
%% and offers improvements in runtime in a parallel environment and better exploration of clustering space. 
%\\
%% compare this to inference performed using MCMC methods and also to \texttt{mclust}, a popular mixture model R package. We show that CC can be extended to Bayesian integrative clustering models. CC is then demonstrated on real datasets in both the single dataset and multiple dataset context. CC is shown to capture more modes in the clustering distribution than either the maximum-likelihood estimate (MLE) or any individual Markov chain. CC also reduces the computation time required when a parallel environment is present compared to Bayesian inference. \\
%%\textbf{Availability:} www.github.com/stcolema/ccbm \\
%\textbf{Contact:} \href{stephen.coleman@mrc-bsu.cam.ac.uk}{stephen.coleman@mrc-bsu.cam.ac.uk}\\
%\textbf{Supplementary information:} Supplementary data are available at \textit{Bioinformatics} online and code at \textit{https://github.com/stcolema/ConsensusClusteringForBayesianMixtureModels}.}
%
%\maketitle
...
%\subsection{Problem}
%However,  performing  inference  in  Bayesian  mixture  models  is  a  difficult  task. Variational inference \citep{blei2006variational} (VI) may be used to perform approximate inference \citep{martin2020computing}, but can struggle with multi-modality, \commentPDWK{is known to} underestimate the variance  in  the  posterior  distribution  \citep{two2011turner}\commentPDWK{, and  can require multiple initialisations} \citep{wang2011fast}. Implementation is difficult, requiring either complex derivations \citep[see the Appendix Supplementary Methods of][for an example]{argelaguet2018multi} or black-box, approximate solutions \citep{kucukelbir2017automatic}.

Markov chain Monte Carlo (MCMC) methods are the most common tool for performing computational Bayesian inference. In Bayesian clustering, they are used to draw a collection of clustering partitions from the posterior distribution.  %construct a chain of clusterings, and then an assessment of the convergence of this chain is made, to determine if its behaviour aligns with the expected asymptotic theory. 
However, in practice, chains can become stuck in local posterior modes preventing convergence %individual chains often fail to explore the full support of the posterior distribution despite the ergodicity of MCMC methods
...
%In contrast to the above, we apply our method to the full dataset, and do not make assumptions about the chains exploring any common space or that our inference has a Bayesian interpretation. Our work is purely heuristic. However, we expect that if each chain provides a better than random estimate of the clustering and there is sufficient diversity across the chains, then the ensemble will provide a meaningful estimate of the clustering,  allowing us to neglect burn-in bias.  Furthermore, our method is a wrapper for Bayesian methods. It may be applied to existing implementations of sampling-based clustering methods in contrast to developments in MCMC algorithms which require a full re-implementation.


% \commentCW{Perhaps could prefix with a clear statement of the specific problem we address, because rev 1 totally missed it and the above paragraphs set out various issues without stating which is the focus. eg "Here, we aim to address the issue of lack of convergence of long MCMC chains, by ..."}
...
%of these implementations    in Bayesian mixture models, we aim to use consensus clustering of Bayesian mixture models to keep the flexibility these methods provide but better describe multiple modes in the likelihood surface and reduce the computational runtime.
 Specifically, we make use of existing sampling-based Bayesian clustering implementations, but only run them for a fixed (and relatively small) number of iterations, stopping before they have converged to their target stationary distribution. Doing this repeatedly, we obtain an ensemble of clustering partitions, which we use to perform consensus clustering. %We propose that applying consensus clustering to Bayesian mixture models can overcome some of the issues endemic in high dimensional Bayesian clustering. \cite{monti2003consensus} suggest this application as part of their original paper, but no investigation has been attempted to our knowledge. This ensemble approach  sidesteps the problems of convergence associated MCMC methods and offers computational gains through using shorter chains run in parallel. Furthermore, this approach could be directly used on any existing MCMC based implementation of Bayesian mixture models or their extensions and would avoid the re-implementation process that changing to newer MCMC methods or VI would entail.
...
%\commentPDWK{Although our procedure is heuristic (and, we stress, non-Bayesian), we demonstrate through simulations and real data analysis that it provides meaningful and interpretable results}. 


% \commentPDWK{Referring to reviewer 2, comment 1. Could you have a go at adding some of this to the intro?  Can add a parargraph before the final paragraph in the intro -- take some bits from the abstract about this being a non-Bayesian approach, and contrast with methods that have been used within the Bayesian formalism, then say why they are not desirable here (pragmatically: it would require a complete reimplmentation of the method, whereas our approach is a wrapper).}


We show via simulation that our approach can successfully identify meaningful clustering structures. %We also show that consensus clustering explores as many or more modes of the likelihood surface than either standard Bayesian inference or \texttt{Mclust}, a maximum likelihood method, all while offering improvements in runtime to traditional Bayesian inference.
...
% \commentSC{We believe that the success of our method is due to the fact that the inferred clustering is based on pairwise interactions, being defined by the coclustering of items within each sampled partition. The coclustering of two items is a discrete variable defined by the label of each of the pair of items. The true coclustering of a number of items is far simpler to infer the the clustering of all items. We therefore expect that short chains are able to cocluster a number of similar points correctly, providing a better than random estimate of the entire clustering, even if we might expect that each individual chain over-estimates the number of clusters (a value defined by entire allocation vector) or has some items incorrectly clustered. Therefore, if the chains are sufficiently diverse in their exploration such that the pairwise interactions across the ensemble covers each true cluster the final estimate will cocluster items correctly. It should be noted that continuous variables or variables defined by a large fraction of the dataset are more likely to have meaningless values derived in the short chains; however as the sample is generated from deeper in the chain we would expect the distribution described by the sampled values for these variables to converge on the posterior distribution, as the results from our use of MDI suggest in figure XX of the supplementary material. We expect that for most continuous or global variables the ensemble estimate would be no more useful than that provided by each chain; however, if the number of clusters present is estimated based on the consensus matrix as it is here, we believe that the estimate should be reasonable if the ensemble is sufficiently large. This is as $K$ is then based on many pairwise coclusterings, and while the variance on this quantity may be large for short chains, we expect that the point estimate should be useful and correspond to the true number of clusters present. The results of our simulation study suggest that this is true.}
