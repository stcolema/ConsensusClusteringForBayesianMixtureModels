...
# MDI output
continuous_data <- readRDS("./Data/Yeast/BayesContParamsTibble.rds")
alloc_data <- readRDS("./Data/Yeast/BayesAllocTibble.rds")

n_chains <- nrow(continuous_data)
...
burn_in <- 1e5
thin <- 1e3
eff_burn <- burn_in / thin
dropped_samples <- seq(1, eff_burn)
R <- 666e3
kept_samples <- seq(burn_in + thin, R, thin)
length(kept_samples)
nrow(as.mcmc(continuous_data$Samples[[i]][dropped_samples, ]))

for (i in 1:n_chains) {
  mcmc_lst[[i]] <- as.mcmc(continuous_data$Samples[[i]][-dropped_samples, ])
  attr(mcmc_lst[[i]], "mcpar") <- c(burn_in + thin + 1, 666e3 + 1, thin)
}

# test_burn_in <- seq(0, 6e6, 1e6)
# n_tested <- length(test_burn_in)
# my_ess <- matrix(0, ncol = n_cont_parameters + 1, nrow = n_tested) %>%
#   set_colnames(c(param_labels, "Burn"))
# # %>%
#   # as.data.frame()
#
# for(b in seq(1, n_tested)) {
#   burn_in <- test_burn_in[b]
#   my_ess[b, ] <- unlist(mdiHelpR::findESS(mcmc_lst, test_burn_in = burn_in))
# }
...
geweke_df_phis_only <- geweke_df %>%
  ...
  filter(Parameter_expr %in% c("phi[12]", "phi[13]", "phi[23]"))

p_geweke <- geweke_df_phis_only %>%
  ...
  facet_wrap(~Seed,
    labeller = as_labeller(chain_labels),
    ncol = 2
  ) +
...
p_gelman <- p_gelman$data %>%
  ...
  facet_wrap(~Parameter, labeller = as_labeller(param_labels, label_parsed)) +
  labs(x = "Last iteration in chain", y = "Shrinkage factor") +
...
for (i in 1:n_chains) {
  .curr_data <- continuous_data$Samples[[i]][-dropped_samples, ] %>%
    pivot_longer(cols = everything(), names_to = "Parameter")

  .curr_data$Chain <- continuous_data$Seed[[i]]
...
cont_plt_data$Chain <- factor(cont_plt_data$Chain)
cont_plt_data <- cont_plt_data %>%
  filter(Parameter %in% c("Phi_12", "Phi_13", "Phi_23"))
...
p_param_density_reduced <- cont_plt_data %>%
  filter(Chain %in% chains_to_keep) %>%
  ggplot(aes(x = value, fill = Chain)) +
  geom_density(alpha = 0.2) +
  facet_wrap(~Parameter, labeller = as_labeller(param_labels, label_parsed)) +
  labs(
...
#   theme(legend.position="bottom")
#
# p_param_density_reduced <- p_param_density_reduced +
#   theme(legend.position="bottom")

p_patch <- (p_geweke + (p_gelman / p_param_density_reduced)) +
  plot_annotation(tag_levels = "A")

# p_patch

ggsave("./SupplementaryMaterial/Images/Yeast/bayesConvergence.png",
  plot = p_patch,
  height = 7, width = 12
)
